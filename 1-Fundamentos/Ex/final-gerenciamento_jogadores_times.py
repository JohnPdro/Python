...
def print_teams() :
    ...
    for i, team in enumerate(teams.values()) : # enumerate é utilizado para criar index para cada valor de teams
        print(f"{i + 1}. {team['name']} ({len(team['players'])} jogadores)")

# função para lista jogadores de um time
...
while not done :
    ...
    if choice == "1" :
        ...
        if team_num <= len(teams) :
            ...
            player_num = int(input("Informe o número de jogador que deseja remover\n"))
